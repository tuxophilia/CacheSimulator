	...
	void map(char *dec_addr,char *op,int bid)
	{
		...
		if (!strcmp(op,"r"))
		{
			...
			{	
				...
				outFile<<"Read Data is in the cache"<<endl;
			...
			{	
				...
				outFile<<"Read Data is not in the cache"<<endl;
		...
		else if(!strcmp(op,"w"))
		{
			if (isHit(bin_addr,getSetIndex(bin_addr)))
			{
				...
				outFile<<"Write Data is in the cache"<<endl;
			...
			else
			{
				...
				outFile<<"Write Data is not in the cache"<<endl;
	...
	while(!inFile.eof()){
		...
		inFile>>addr>>opr;
	 	...
	 	if(charEqual(file,tid,1))
		 	l1.bank[(args->thread_num)-1].map(addr,opr,args->thread_num);  // Since 
	 	/*
