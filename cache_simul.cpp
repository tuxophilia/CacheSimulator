...
int cache_size=0,bank_size=0,block_size=0,way=0,num_of_bank=0,pointer;
...
class cache_bank{ //Create one bank of l1 cache structure 
	...
	char* tostr (int x)//convert the integer to string
	{
	...
	void map(char *dec_addr,char *op,int bid)
	{
		...
		ofstream outFile,misslog;
		char output[]="output_bank_";
		strcat(output,tostr(bid));
		strcat(output,suffix);
		outFile.open(output,fstream::app);
		
		misslog.open("miss_log.txt",fstream::app);
		
		if (outFile.fail()) /// In case of file not opned
		{
			cerr<<"Error while opening output file"<<endl;
			exit(1);
		}
		
		if (misslog.fail()) /// In case of file not opned
		{
			cerr<<"Error while opening output file"<<endl;
			exit(1);
		}
		...
		if (!strcmp(op,"r"))
		{
			...
			{	
				cout<<"\nHIT: Read Data is in the cache"<<endl;
				cout<<"Address: "<<dec_addr<<" Reading Block data from set: "<<setIndex<<"\tBank Id: "<<bid<<endl;
				outFile<<"\nHIT: Read Data is in the cache"<<endl;
				outFile<<"Address: "<<dec_addr<<" Reading Block data from set: "<<setIndex<<"\tBank Id: "<<bid<<endl;
			...
			{	
				cout<<"\nMISS: Read Data is not in the cache"<<endl;
				...
				outFile<<"\nMISS: Read Data is not in the cache"<<endl;
				outFile<<"Reading request but found Read Miss Data will be loaded and writen to following addresses:"<<endl;
				outFile<<""<<"Address: "<<dec_addr<<"Loading Block data to set: "<<setIndex<<"\tBank Id: "<<bid<<endl;
				//cout<<"Read Miss at the Address: "<<dec_addr<<":=\t"<<bin_addr<<"\tBank Id: "<<bid<<endl;
				//pthread_mutex_lock(&myMutex);
				misslog<<"Read Miss at the Address: "<<dec_addr<<"\tBank Id: "<<bid<<endl;
...
struct arg_struct {
 ...
 void *make_thread(void *threadarg){
	...
	do{
		...
		tid[0]=(char)((args->thread_num)+48);

		inFile>>addr>>opr;
		
 		cout<<addr<<" "<<opr<<" "<<tid<<"\n";
	 	cout<<charEqual(file,tid,1);
	 	if(charEqual(file,tid,1))
		 	l1.bank[(args->thread_num)-1].map(addr,opr,args->thread_num);
	 	/*
